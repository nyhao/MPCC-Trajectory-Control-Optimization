...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )  
{
...
#endif	 
    copyMArrayToC(mxGetPr(par), params.x0, 330);
    ...
    if( mxGetM(par) != 25 || mxGetN(par) != 1 ) {
    mexErrMsgTxt("PARAMS.xinit must be of size [25 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.xinit, 25);
	...
	copyCArrayToM( output.x01, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x02, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x03, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x04, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x05, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x06, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x07, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x08, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x09, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x10, mxGetPr(outvar), 30);
	mxSetField(plhs[0], 0, "x10", outvar);

	outvar = mxCreateDoubleMatrix(30, 1, mxREAL);
	copyCArrayToM( output.x11, mxGetPr(outvar), 30);
