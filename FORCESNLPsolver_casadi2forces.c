                  ...
                  )
{
    ...
    out[1] = nabla_f_sparse;
	 ...
	 if (stage >= 0 && stage < 10) && (stage >= 11 && stage < 40)
	 {
		 /* set inputs */
		 out[2] = c_sparse;
	 ...
	 if (stage >= 10 && stage < 11)
	 {
		 ...
		 if( c ){ FORCESNLPsolver_model_11_sparsity(4, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, c_sparse, c); }
		 if( nabla_c ){ FORCESNLPsolver_model_11_sparsity(5, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, nabla_c_sparse, nabla_c); }
