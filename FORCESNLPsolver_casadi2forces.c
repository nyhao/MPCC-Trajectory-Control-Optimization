...
extern void FORCESNLPsolver_model_21(const double** arg, double** res);
extern void FORCESNLPsolver_model_21_sparsity(int i, int *nrow, int *ncol, const int **colind, const int **row);
                  ...
                  )
{
    ...
    out[1] = nabla_f_sparse;
	 ...
	 {
		 /* call CasADi */
		 FORCESNLPsolver_model_21(in, out);

		 /* copy to dense */
		 if( nabla_f ){ FORCESNLPsolver_model_21_sparsity(3, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, nabla_f_sparse, nabla_f); }
